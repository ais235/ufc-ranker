    ...
    def find_fighter_by_name(self, cursor, fighter_name: str) -> Optional[int]:
        ...
        try:
            cursor.execute("""
                ...
                WHERE name_en = ? OR full_name = ? OR name_ru = ?
                ORDER BY 
                    ...
                    id
                LIMIT 1
            """, (fighter_name, fighter_name, fighter_name, fighter_name, fighter_name))
    ...
    def update_rankings_in_database(self, rankings_data):
        ...
        try:
            ...
            for weight_class, rankings in rankings_data.items():
                ...
                db_weight_class = self.weight_class_mapping.get(weight_class, weight_class)
                
                # Получаем ID весовой категории
                weight_class_id = weight_classes.get(db_weight_class)
                if not weight_class_id:
                    logger.warning(f"Весовая категория {db_weight_class} не найдена в БД")
                    continue
